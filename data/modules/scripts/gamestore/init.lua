...
function parseBuyStoreOffer(playerId, msg)
	...
	if (table.contains(GameStore.OfferTypes, offer.type) == false)                      -- we've got an invalid offer type
		...
		or (offer.type ~= GameStore.OfferTypes.OFFER_TYPE_NAMECHANGE and
			...
			offer.type ~= GameStore.OfferTypes.OFFER_TYPE_SKILLBOOST and
	not offer.id) then
	...
	local pcallOk, pcallError = pcall(function()
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_SKILLBOOST       then GameStore.processSkillBoostPuchase(player, offer.count)
...
function sendStorePurchaseSuccessful(playerId, message)
	...
	print("STORE LOG")
	--MARSON
	local file = io.open("data/logs/store.log", "a")
	if not file then
		return
	end
	...
	local accountName
		local resultId = db.storeQuery("SELECT `name` FROM `accounts` WHERE `id` = " .. db.escapeString(player:getAccountId()))
		if resultId ~= false then
			accountName = result.getString(resultId, "name")
			result.free(resultId)
		end
	io.write("CHAR: "..player:getName().." ACC: "..accountName.." MESSAGE: "..message.." \n")
	io.close(file)
	--MARSON
...
GameStore.getDefaultDescription = function(offerType, count)
	...
	elseif offerType == t.OFFER_TYPE_SKILLBOOST then
		descList = GameStore.DefaultDescriptions.SKILLBOOST
...
function GameStore.processSkillBoostPuchase(player, count) 
	local tempo = 60 * 60 * count
			
	local check = player:getStorageValue(25897)
	local skilltime = check - os.time()
	if skilltime > 0 then
	setPlayerStorageValue(player, 25897, os.time()+tempo+skilltime)
	--Criação do cache de otimização:
	SKILL_BOOST_CACHE[player:getGuid()] = os.time()+tempo+skilltime
	else
	
	setPlayerStorageValue(player, 25897, os.time()+tempo)
	--Criação do cache de otimização:
	SKILL_BOOST_CACHE[player:getGuid()] = os.time()+tempo
	end
