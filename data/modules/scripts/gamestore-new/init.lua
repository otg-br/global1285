...
GameStore.OfferTypes = {
	...
	OFFER_TYPE_PREMIUM1 = 25,
	OFFER_TYPE_PREYBONUS_ONLINE = 26
}
...
local function checkNameHireling(playerId)
	local resultId = db.storeQuery("SELECT `name` FROM `player_hirelings` WHERE `player_id` = " .. playerId)
	if resultId ~= false then
        local guid = result.getDataString(resultId, "name")
...
GameStore.DefaultDescriptions = {
	...
	NAMECHANGE  = { "Are you hunted? Tired of that? Get a new name, a new life!",
 	...
 	PREYBONUSONLINE = { "You activated a prey but do not like the randomly selected bonus? Roll for a new one! Here you can purchase five Prey Bonus Rerolls! \nA Bonus Reroll allows you to get a bonus with a higher value (max. 50%). The bonus for your prey will be selected randomly from one of the following: damage boost, damage reduction, bonus XP, improved loot. The 2 hours hunting time will start anew once you have rolled for a new bonus. Your prey monster will stay the same." },
...
function parseRequestStoreOffers(playerId, msg)
	...
	if actionType == GameStore.ActionType.OPEN_CATEGORY then
		...
		if string.find(categoryName, 'Online') then --marson
		sendUpdateCoinBalance(playerId, 1)
		print("Chamei o saldo TOURNAMENT")
		else
		print("Chamei o saldo PREMIUM")
...
function parseBuyStoreOffer(playerId, msg)
	...
	if (table.contains(GameStore.OfferTypes, offer.type) == false)                      -- we've got an invalid offer type
		...
		or (offer.type ~= GameStore.OfferTypes.OFFER_TYPE_NAMECHANGE and
			...
			offer.type ~= GameStore.OfferTypes.OFFER_TYPE_PREYBONUS_ONLINE and
	...
	--Aqui checamos se o player tem os coins necessários JOSE
        if offer.coinType == 2 then
         if player:getStorageValue(Storage.TournamentCoins) < offerPrice then
		return queueSendStoreAlertToUser("You don't have enough tournament coins. Your purchase has been cancelled.", 250, playerId)
          end
	...
	local pcallOk, pcallError = pcall(function()
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_PREMIUM1        then GameStore.processPremiumPurchase1(player, offer.id)																													   
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_PREYBONUS_ONLINE      then GameStore.processPreyBonusRerollOnline(player, offer.count)
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_HIRELING       then 
			...
			local sex = 1 
			if msg:getByte() == 1 then 
				sex = 1 
			else 
				sex = 0 
			end
			if checkNameHireling(player:getId()) ~= hirelingName then
	...
	if configure ~= GameStore.ConfigureOffers.SHOW_CONFIGURE then
		if offer.coinType == 2 then
		player:setStorageValue(Storage.TournamentCoins, player:getStorageValue(Storage.TournamentCoins) - offerPrice)
		else
		player:removeCoinsBalance(offerPrice) --gsi
		end
		...
		if offer.coinType == 2 then
		message = string.format("You have purchased %s for %d Tournament coins.", offer.name, offerPrice)
		end
		sendUpdateCoinBalance(playerId)
...
function Player.canBuyOffer(self, offer)
	...
	if disabled ~= 1 then
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_PREYBONUS then
			if self:getBonusRerollCount() >= 50 then
				disabled = 1
...
function sendUpdateCoinBalance(playerId, param)
	...
	if player:getClient().version > 1200 then
	  ...
	  msg:addU32(player:getCoinsBalance())
          ...
          if param == 1 then
			msg:addU32(player:getStorageValue(Storage.TournamentCoins))
          else
          msg:addU32(player:getCoinsBalance())
          end
	  msg:addU32(player:getCoinsBalance())
	...
	if player:getClient().version > 1200 then
		--msg:addU32(player:getCoinsBalance())
		if player:getStorageValue(Storage.TournamentCoins) < 0 then
		player:setStorageValue(Storage.TournamentCoins, 0)
		end
		msg:addU32(player:getStorageValue(Storage.TournamentCoins)) -- Tournament Coins
	end
...
GameStore.getDefaultDescription = function(offerType, count)
	...
	elseif offerType == t.OFFER_TYPE_PREYBONUS then
		...
		elseif offerType == t.OFFER_TYPE_PREYBONUS_ONLINE then
		descList = GameStore.DefaultDescriptions.PREYBONUSONLINE
...
function GameStore.processItemPurchase(player, offerId, offerCount)
	...
	if inbox and inbox:getEmptySlots() > offerCount then
		for t = 1, offerCount do
			local item = inbox:addItem(offerId, offerCount or 1)
...
function GameStore.processPremiumPurchase1(player, offerId)
	player:addInfiniteVip()					
end
...
function GameStore.processPreyBonusReroll(player, offerCount)
	if player:getBonusRerollCount() + offerCount >= 51 then
...
function GameStore.processPreyBonusRerollOnline(player, offerCount)
	if player:getBonusRerollCount() + offerCount >= 51 then
		return error({code = 1, message = "You cannot own more than 50 prey wildcards."})
	end
	player:setBonusRerollCount(player:getBonusRerollCount() + offerCount)
