...
function Player:onLook(thing, position, distance)
	...
	if thing:isItem() then
		...
		if thing:isPlayer() then
			description = string.format("%s\nDrop Rate: level %d", description, thing:getStorageValue(Storage.DropRate))
		end
		--MARSON
	end
...
function Player:onGainSkillTries(skill, tries)
	...
	--Skill Boost Check
	local skilltime = 0
	if not SKILL_BOOST_CACHE[self:getGuid()] then --ainda nao criamos a tabela...criaremos com um valor...Se houver tempo, o valor será positivo.. caso contrário será zero. caso seja comprado pela store, adicionar um valor positivo
		local check = self:getStorageValue(25897) --Vamos ler a storage pela primeira vez
		skilltime = check - os.time()
		if skilltime > 0 then --como o valor foi positivo, gravar na tabela
			SKILL_BOOST_CACHE[self:getGuid()] = check
			else
			SKILL_BOOST_CACHE[self:getGuid()] = 0 --como o valor foi nulo, ou negativo, gravaremos 0 no cache para impedir uma proxima leitura
		end
	else --nesse caso a tabela ja foi criada..ela vai conter o tempo em os.time...
		if SKILL_BOOST_CACHE[self:getGuid()] < os.time() then --aqui, estamos comparando o cache, com o valor os.time... durante o bonus ativo o cache será maior que o os.time, após a expiração a variável skilltime voltará a ser 0, por definição
			skilltime = 1 --caso seja comprado pela store (após ter sido criado um cache zerado) será necessário regravar o valor em cache
		end	
	end
	if skilltime > 0 then
		tries = tries * 2
